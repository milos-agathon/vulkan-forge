...
class WaveSimulation:
    ...
    def step(self, dt=0.1):
        ...
        # Update wave
        self.u_next = (
            2 * self.u - self.u_prev + self.c**2 * dt**2 * laplacian
        ) * self.damping
...
def main():
    ...
    for frame in range(120):  # 20 seconds at 30 FPS
        ...
        if frame % 30 == 0:
            ...
            print(
                f"Frame {frame}: Total: {avg_frame:.2f}ms "
    ...
    print(
        f"Sync bandwidth: {(vertices.nbytes + colors.nbytes) / 1024 / 1024 / (np.mean(sync_times) / 1000):.0f} MB/s"
    )

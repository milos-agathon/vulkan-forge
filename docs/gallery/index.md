...
CI runs required recipe pixel goldens in the `Visual Goldens (NVIDIA Vulkan)` job. The candidate path installs `pyproj`, proves a physical NVIDIA/Vulkan adapter with `python scripts/terrain_ci_probe.py --mode terrain --require-nvidia-vulkan`, then runs terrain and recipe image comparisons with `FORGE3D_RUN_TERRAIN_GOLDENS=1`. An absent adapter, skipped test, or pixel mismatch is fatal. Candidate code never receives production signing material; signed certificate rotation remains the protected, main-only `Refresh recipe certificates` workflow. Ordinary pull-request and push runs do not select Apple Metal acceptance. PR #170's required physical Apple/Metal evidence belongs to the dedicated `Apple Metal Acceptance` family selected by the nightly schedule or manual `scope=full`; generic Metal diagnostics, an absent adapter, and software fallback do not satisfy it. `FORGE3D_ALLOW_SOFTWARE_GOLDENS=1` remains scoped to local recipe meta-validation and must never be treated as the render-quality gate.

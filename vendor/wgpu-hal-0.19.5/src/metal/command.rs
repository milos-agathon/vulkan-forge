...
use std::sync::Arc;
use std::{borrow::Cow, mem, ops::Range, sync::atomic};
...
impl super::CommandEncoder {
    fn create_raw_command_buffer(
        &self,
        label: Option<&str>,
        retain_references: bool,
    ) -> Result<metal::CommandBuffer, crate::DeviceError> {
        let previous = self
            .queue_shared
            .command_buffer_created_not_submitted
            .fetch_add(1, atomic::Ordering::AcqRel);
        if previous >= super::adapter::MAX_COMMAND_BUFFERS as usize {
            self.queue_shared
                .command_buffer_created_not_submitted
                .fetch_sub(1, atomic::Ordering::AcqRel);
            log::warn!(
                "metal: refusing to create a command buffer; {} outstanding native buffers \
                 reaches the limit of {}",
                previous + 1,
                super::adapter::MAX_COMMAND_BUFFERS,
            );
            return Err(crate::DeviceError::Lost);
        }

        let raw = objc::rc::autoreleasepool(|| {
            let queue = self.queue_shared.raw.lock();
            let raw = if retain_references {
    ...
    fn release_unsubmitted_command_buffer(&self) {
        super::decrement_unsubmitted(&self.queue_shared);
    }

    fn close_debug_groups_on_current_buffer(&self) {
        if let Some(raw) = self.raw_cmd_buf.as_ref() {
            for _ in &self.debug_marker_stack {
                raw.pop_debug_group();
            }
        }
    }

    fn reopen_debug_groups_on_current_buffer(&self) {
        if let Some(raw) = self.raw_cmd_buf.as_ref() {
            for group in &self.debug_marker_stack {
                raw.push_debug_group(group);
            }
        }
    }

    fn encode_mac26_timestamp(
        &mut self,
        set: &super::QuerySet,
        index: u32,
        ...
        self.leave_blit();
        let raw = self.raw_cmd_buf.as_ref().ok_or(crate::DeviceError::Lost)?;
        let target = set
            .timestamp_dummy_target
            .as_ref()
        ...
        let sample_buffer = set
            .counter_sample_buffer
            .as_ref()
            .ok_or(crate::DeviceError::ResourceCreationFailed)?;
        sample_attachment.set_sample_buffer(sample_buffer);
        sample_attachment.set_start_of_vertex_sample_index(index as _);
        sample_attachment.set_end_of_vertex_sample_index(metal::COUNTER_DONT_SAMPLE);
        sample_attachment.set_start_of_fragment_sample_index(metal::COUNTER_DONT_SAMPLE);
        sample_attachment.set_end_of_fragment_sample_index(metal::COUNTER_DONT_SAMPLE);

        let encoder = raw.new_render_command_encoder(&descriptor).to_owned();
        encoder.set_label("(wgpu internal) Timestamp sample");
        encoder.end_encoding();
        Ok(())
    }

    fn defer_timestamp_resolve(
        &mut self,
        set: &super::QuerySet,
        range: Range<u32>,
        buffer: &super::Buffer,
        ...
        let resolve_raw =
            self.create_raw_command_buffer(Some("(wgpu internal) Timestamp resolve"), true)?;
        let resolve_blit = resolve_raw.new_blit_command_encoder().to_owned();
        let continuation = match self
            .create_raw_command_buffer(Some("(wgpu internal) Timestamp resolve continuation"), true)
        {
            Ok(raw) => raw,
            Err(error) => {
                resolve_blit.end_encoding();
                self.release_unsubmitted_command_buffer();
                return Err(error);
            }
        };

        self.close_debug_groups_on_current_buffer();
        self.leave_blit();
        let predecessor = self.raw_cmd_buf.take().ok_or_else(|| {
            resolve_blit.end_encoding();
            self.release_unsubmitted_command_buffer();
            self.release_unsubmitted_command_buffer();
        ...
        self.completed_segments
            .push(super::CommandBufferSegment::Commands {
                raw: predecessor.clone(),
                deferred: self.raw_cmd_buf_deferred,
            });
        self.completed_segments
            .push(super::CommandBufferSegment::DeferredTimestampResolve(
                Arc::new(super::DeferredTimestampResolve {
                    predecessor,
                    raw: resolve_raw,
                    blit: resolve_blit,
                    ...
                    sample_buffer,
                    range,
                    destination: buffer.raw.to_owned(),
                    offset,
                    debug_groups: self.debug_marker_stack.clone(),
                    ended: atomic::AtomicBool::new(false),
                }),
            ));
        self.raw_cmd_buf = Some(continuation);
        self.raw_cmd_buf_deferred = true;
        self.reopen_debug_groups_on_current_buffer();
        Ok(())
    }
...
impl crate::CommandEncoder<super::Api> for super::CommandEncoder {
    unsafe fn begin_encoding(&mut self, label: crate::Label) -> Result<(), crate::DeviceError> {
        ...
        self.raw_cmd_buf = Some(self.create_raw_command_buffer(label, retain_references)?);
        self.raw_cmd_buf_deferred = false;
        self.reopen_debug_groups_on_current_buffer();
    ...
    unsafe fn discard_encoding(&mut self) {
        ...
        self.debug_marker_stack.clear();
        let command_buffer_count =
            self.completed_segments.len() + usize::from(self.raw_cmd_buf.is_some());
    ...
    unsafe fn end_encoding(&mut self) -> Result<super::CommandBuffer, crate::DeviceError> {
        if let Some(error) = self.recording_error.take() {
        ...
        Ok(super::CommandBuffer {
            segments: mem::take(&mut self.completed_segments),
    ...
    unsafe fn write_timestamp(&mut self, set: &super::QuerySet, index: u32) {
        ...
        if self
            .shared
            .private_caps
            .timestamp_query_requires_render_boundary
        {
            // macOS 26 reports the old imperative sampling capabilities but
            // writes zero through them.  A render-pass sample attachment at
            // the start-of-vertex boundary remains operational.  Keep every
            // logical query at its own index and disable all other boundaries.
            if self.recording_error.is_none() {
                if let Err(error) = self.encode_mac26_timestamp(set, index) {
    ...
    unsafe fn copy_query_results(
        ...
        match set.ty {
            ...
            {
                if self.recording_error.is_none() {
                    if let Err(error) = self.defer_timestamp_resolve(set, range, buffer, offset) {
                        self.recording_error = Some(error);
                    }
                }
            }
    ...
    unsafe fn begin_debug_marker(&mut self, group_label: &str) {
        if self
            ...
            .timestamp_query_requires_render_boundary
        {
            if let Some(encoder) = &self.state.render {
                encoder.push_debug_group(group_label);
                return;
            }
            if let Some(encoder) = &self.state.compute {
                encoder.push_debug_group(group_label);
                return;
            }
            self.leave_blit();
            if let Some(ref buf) = self.raw_cmd_buf {
                buf.push_debug_group(group_label);
                self.debug_marker_stack.push(group_label.to_owned());
            }
            return;
        }
        if let Some(encoder) = self.active_encoder() {

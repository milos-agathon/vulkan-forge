...
impl crate::Device<super::Api> for super::Device {
    ...
    unsafe fn create_command_encoder(
        ...
        Ok(super::CommandEncoder {
            ...
            raw_cmd_buf: None,
            raw_cmd_buf_deferred: false,
            completed_segments: Vec::new(),
            recording_error: None,
            debug_marker_stack: Vec::new(),
        ...
        objc::rc::autoreleasepool(|| {
            match desc.ty {
                ...
                wgt::QueryType::Timestamp => {
                    ...
                    let timestamp_dummy_target = if self
                        .shared
                        .private_caps
                        .timestamp_query_requires_render_boundary
                    {
                        let target_descriptor = metal::TextureDescriptor::new();
                        target_descriptor.set_texture_type(metal::MTLTextureType::D2);
                        target_descriptor.set_width(1);
                        target_descriptor.set_height(1);
                        target_descriptor.set_mipmap_level_count(1);
                        target_descriptor.set_pixel_format(metal::MTLPixelFormat::RGBA8Unorm);
                        ...
                        if target.as_ptr().is_null() {
                            return Err(crate::DeviceError::OutOfMemory);
                        }
                        target.set_label("(wgpu internal) Timestamp sample target");
                        Some(target)
                    } else {
                        None
                    };
    ...
    unsafe fn create_fence(&self) -> DeviceResult<super::Fence> {
        Ok(super::Fence {
            ...
            async_device_error: Arc::new(atomic::AtomicU8::new(super::ASYNC_DEVICE_ERROR_NONE)),
    ...
    unsafe fn get_fence_value(&self, fence: &super::Fence) -> DeviceResult<crate::FenceValue> {
        ...
        Ok(fence.completed_value.load(atomic::Ordering::Acquire))
    }
    unsafe fn wait(
        &self,
        fence: &super::Fence,
        ...
        loop {
            ...
            if cmd_buf.status() == metal::MTLCommandBufferStatus::Error {
                return Err(super::command_buffer_device_error(cmd_buf));
            }

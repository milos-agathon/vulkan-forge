...
pub(super) const MAX_COMMAND_BUFFERS: u64 = 2048;
...
impl crate::Adapter<super::Api> for super::Adapter {
    unsafe fn open(
        ...
        Ok(crate::OpenDevice {
            ...
            queue: super::Queue {
                shared: super::QueueShared::new(queue),
                timestamp_period,
            },
        })
    }
...
impl super::PrivateCapabilities {
    ...
    pub fn new(device: &metal::Device) -> Self {
        ...
        // macOS 26 still reports the legacy stage/draw/dispatch/blit counter
        // capabilities, but the imperative sampling entry points produce
        // zeros on affected Apple Silicon.  The render-pass boundary form is
        // the only legacy form proven to populate timestamps there.
        let timestamp_query_requires_render_boundary =
            os_is_mac && version.at_least((26, 0), OS_NOT_SUPPORT, os_is_mac);
            ...
            && device.supports_counter_sampling(metal::MTLCounterSamplingPoint::AtStageBoundary)
        {
            ...
            if !timestamp_query_requires_render_boundary {
